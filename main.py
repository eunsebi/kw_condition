...
class KiwoomConditon(QObject):
    ...
    # 매도로 종목 정보 삭제 할시 쓰레드로 인해 삭제된 자료에 접근하는 것을 방지위해 만듬
    sigRemoveJongmokInfo = pyqtSignal(str)
    ...
    def __init__(self):
        ...
        # { 'yupjong_code': { '현재가': 222, ...} }
        self.upjongInfo = {'코스피': {}, '코스닥': {}}
        self.jangoInfo = {}  # { 'jongmok_code': { '이익실현가': 222, ...}}
        self.jangoInfoFromFile = {}  # TR 잔고 정보 요청 조회로는 얻을 수 없는 데이터를 파일로 저장하고 첫 실행시 로드함
        # { '날짜' : [ [ '주문구분', '매도', '분할매수이력', '체결가' , '체결수량', '미체결수량'] ] }
        self.chegyeolInfo = {}
        # 조건 진입이 발생한 모든 리스트 저장하고 매수 결정에 사용되는 모든 정보를 저장함  [ {'종목코드': code, ...}]
        self.conditionOccurList = []

        self.conditionStoplossList = {
            '1분': [], '3분': [], '15분': [], '30분': []}  # 기존 조건 진입후 손절 조건을 판단하기 위함
    ...
    def createState(self):
        ...
        initSystemState.addTransition(
            self.sigGetConditionCplt, requestingJangoSystemState)
        requestingJangoSystemState.addTransition(
            self.sigRequestJangoComplete, waitingTradeSystemState)

        waitingTradeSystemState.addTransition(
            self.sigSelectCondition, standbySystemState)

        standbySystemState.addTransition(
            self.sigTerminating,  terminatingSystemState)
        standbySystemState.addTransition(
            self.sigReselectCondition, waitingTradeSystemState)
        standbySystemState.addTransition(
            self.sigRequestRealInfo, beforeProcessStoplossState)

        beforeProcessStoplossState.addTransition(
            self.sigStartProcess, processStoplossState)

        processStoplossState.addTransition(
            self.sigProcessStoploss, processStoplossState)
        ...
        waitingTradeSystemState.entered.connect(
            self.waitingTradeSystemStateEntered)
        requestingJangoSystemState.entered.connect(
            self.requestingJangoSystemStateEntered)
        standbySystemState.entered.connect(self.standbySystemStateEntered)
        beforeProcessStoplossState.entered.connect(
            self.beforeProcessStoplessStateEntered)
        processStoplossState.entered.connect(self.processStoplossStateEntered)
        terminatingSystemState.entered.connect(
            self.terminatingSystemStateEntered)
        ...
        initProcessBuyState.addTransition(
            self.sigStartProcess, standbyProcessBuyState)

        standbyProcessBuyState.addTransition(
            self.sigConditionOccur, standbyProcessBuyState)
        standbyProcessBuyState.addTransition(
            self.sigRequestEtcInfo, requestEtcInfoProcessBuyState)
        standbyProcessBuyState.addTransition(
            self.sigStopProcessBuy, initProcessBuyState)

        requestEtcInfoProcessBuyState.addTransition(
            self.sigWaitTr, waitingTRlimitProcessBuyState)
        requestEtcInfoProcessBuyState.addTransition(
            self.sigDetermineBuy, determineBuyProcessBuyState)
        requestEtcInfoProcessBuyState.addTransition(
            self.sigError, standbyProcessBuyState)

        determineBuyProcessBuyState.addTransition(
            self.sigNoWaitTr, standbyProcessBuyState)
        determineBuyProcessBuyState.addTransition(
            self.sigWaitTr, waitingTRlimitProcessBuyState)

        waitingTRlimitProcessBuyState.addTransition(
            self.sigTrWaitComplete, standbyProcessBuyState)
        ...
        standbyProcessBuyState.entered.connect(
            self.standbyProcessBuyStateEntered)
        requestEtcInfoProcessBuyState.entered.connect(
            self.requestEtcInfoProcessBuyStateEntered)
        determineBuyProcessBuyState.entered.connect(
            self.determineBuyProcessBuyStateEntered)
        waitingTRlimitProcessBuyState.entered.connect(
            self.waitingTRlimitProcessBuyStateEntered)
    ...
    def onRealInfoArrived(self, jongmok_code, real_data_type, result_list):
        ...
        if(jongmok_code in self.jangoInfo):
            self.setRealData(
                real_data_type, self.jangoInfo[jongmok_code], result_list)
        # 조건 발생 종목 정보 update
    ...
    def createConnection(self):
        ...
        self.timerD2YesugmRequest.timeout.connect(
            lambda: self.requestOpw00005(self.account_list[0]))
    ...
    def waitingTradeSystemStateEntered(self):
        ...
        for number, condition in tempDict.items():
            condition_name_screenNo_dict[condition] = [
                kw_util.sendConditionScreenNo + '{}'.format(int(number)), number]
        ...
        for name, info in condition_name_screenNo_dict.items():
            print("8---------------2")
            print("condition_name_screenNo_dict : ",
                  condition_name_screenNo_dict)
            print("name : ", name)
            ...
            if (name == self.current_condition_name):
                ...
                if('이탈' in name):
                    ...
                    print("8---------------3")
                    print("stop condition " + name + ", screen_no: " +
                          info[0] + ", nIndex " + '{}'.format(int(info[1])))
        ...
        for count in range(len(start_info_list)):
            print("start condition " + start_name_list[count] + ", screen_no: " +
                  start_info_list[count][0] + ", nIndex " + '{}'.format(int(start_info_list[count][1])))
            self.sendCondition(start_info_list[count][0], start_name_list[count], int(
                start_info_list[count][1]), 1)
    ...
    def determineBuyProcessBuyStateEntered(self):
        ...
        current_price = kw_util.getHogaPrice(
            current_price, 1, jongmok_jang_type)
        ...
        current_upjong_price = abs(
            round(float(self.upjongInfo[key_upjong_name]['현재가']), 2))
        open_upjong_price = abs(
            round(float(self.upjongInfo[key_upjong_name]['시가']), 2))

        candle_list = self.upjongInfo[key_upjong_name]['분봉']
        price_list = [abs(round(float(item.split(':')[0]), 2))
                      for item in candle_list]
        # 분봉데이터는 소숫점 둘째자리까지 표현되는데 * 100 한 값의 문자열임
        _upjong_20_candle_avr = round(
            ((sum(price_list[1:20]) / 100) + current_upjong_price) / 20, 2)
        ...
        if(jongmok_code not in self.jangoInfo):
            ...
            start_time = datetime.time(
                hour=9, minute=user_setting.REQUEST_MINUTE_CANDLE_TYPE * 2)
        ...
        # 추가 매수시만 적용되는 조건
            ...
            first_bunhal_maesu_time_str = bunhal_maesu_info_list[0].split(':')[
                0]  # 날짜:가격:수량
            first_maeip_price = int(
                bunhal_maesu_info_list[0].split(':')[1])  # 날짜:가격:수량

            # 날짜:가격:수량
            last_bunhal_maesu_time_str = bunhal_maesu_info_list[-1].split(':')[
                0]
            last_maeip_price = int(
                bunhal_maesu_info_list[-1].split(':')[1])  # 날짜:가격:수량

            first_bunhal_maesu_date = datetime.datetime.strptime(
                first_bunhal_maesu_time_str, '%Y%m%d%H%M%S').date()
            last_bunhal_maesu_date = datetime.datetime.strptime(
                last_bunhal_maesu_time_str, '%Y%m%d%H%M%S').date()
        ...
        if(return_vals.count(False) == 0 and current_price != 0):
            ...
            if(user_setting.TEST_MODE == True):
                ...
                if(jongmok_code in self.jangoInfo):
                    ...
                    if(len(bunhal_maesu_list)):
                        first_chegyeol_time_str = bunhal_maesu_list[0].split(':')[
                            0]  # 날짜:가격:수량

                    if(first_chegyeol_time_str != ''):
                        base_time = datetime.datetime.strptime(
                            "20200806010101", "%Y%m%d%H%M%S")

                        first_maesu_time = datetime.datetime.strptime(
                            first_chegyeol_time_str, "%Y%m%d%H%M%S")
            ...
            QTimer.singleShot(
                1000, lambda: self.removeProhibitList(jongmok_code))
    ...
    def sendorder_multi(self, rQName, screenNo, accNo, orderType, code, qty, price, hogaGb, orgOrderNo):
        def inner():
            self.sendOrder(rQName, screenNo, accNo, orderType,
                           code, qty, price, hogaGb, orgOrderNo)
        return inner

    def printStockInfo(self, jongmok_code='all'):
        if(jongmok_code == 'all'):
            print(json.dumps(self.jangoInfo,
                  ensure_ascii=False, indent=2, sort_keys=True))
        else:
            print(json.dumps(
                self.jangoInfo[jongmok_code], ensure_ascii=False, indent=2, sort_keys=True))
        pass

    def printChegyeolInfo(self, current_date='all'):
        if(current_date == 'all'):
            print(json.dumps(self.chegyeolInfo,
                  ensure_ascii=False, indent=2, sort_keys=True))
        elif(current_date == ''):
            current_date = self.currentTime.date().strftime("%y%m%d")
            if(current_date in self.chegyeolInfo):
                print(json.dumps(
                    self.chegyeolInfo[current_date], ensure_ascii=False, indent=2, sort_keys=True))

    def printYupjongInfo(self):
        print(json.dumps(self.upjongInfo,
              ensure_ascii=False, indent=2, sort_keys=True))
    ...
    def requestOpw00004(self, account_num):
        ...
        ret = self.commRqData('{}_opw00004'.format(
            account_num), "opw00004", 0, kw_util.sendYesuGmInfoScreenNo)
        errorString = None
        if(ret != 0):
    ...
    def requestOpw00005(self, account_num):
        ...
        ret = self.commRqData('{}_opw00005'.format(
            account_num), "opw00005", 0, kw_util.sendChegyeolJangoInfoScreenNo)
    ...
    def requestOpw00018(self, account_num, sPrevNext):
        ...
        ret = 0
        if(sPrevNext == "2"):
            ret = self.commRqData(account_num, "opw00018",
                                  2, kw_util.sendAccountInfoScreenNo)
        else:
            ret = self.commRqData(account_num, "opw00018",
                                  0, kw_util.sendAccountInfoScreenNo)
    ...
    def requestOpt10081(self, jongmok_code):
        ...
        ret = self.commRqData(jongmok_code, "opt10081",
                              0, kw_util.sendGibonScreenNo)
        errorString = None
        if(ret != 0):
    ...
    def makeOpt10081Info(self, rQName):
        ...
        jongmok_info_dict['{}일봉중저가'.format(
            user_setting.STOP_LOSS_CALCULATE_DAY)] = min(low_price_list)
        jongmok_info_dict['일{}봉'.format(user_setting.MAX_SAVE_CANDLE_COUNT)
                          ] = total_current_price_list[0:user_setting.MAX_SAVE_CANDLE_COUNT]
    ...
    def requestOpt10080(self, jongmok_code):
     ...
     # 분봉 tr 요청의 경우 너무 많은 데이터를 요청하므로 한개씩 수행
        candle_type_str = "{}:{}분".format(
            user_setting.REQUEST_MINUTE_CANDLE_TYPE)
        ...
        ret = self.commRqData(jongmok_code, "opt10080",
                              0, kw_util.send5minScreenNo)
        errorString = None
        if(ret != 0):
            errorString = jongmok_code + " commRqData() " + kw_util.parseErrorCode(str(ret))
            print(util.whoami() + errorString)
    ...
    def makeOpt10080Info(self, rQName):
        ...
        for i in range(min(repeatCnt, user_setting.MAX_SAVE_CANDLE_COUNT)):
            ...
            for item_name in kw_util.dict_jusik['TR:분봉']:
                ...
                if(item_name == "체결시간"):
                    # 20191104145500 형식
                    if(i == 0):
                        jongmok_info_dict['최근{}분봉체결시간'.format(
                            user_setting.REQUEST_MINUTE_CANDLE_TYPE)] = result.strip()
        ...
        key_minute_candle = '{}분{}봉'.format(
            user_setting.REQUEST_MINUTE_CANDLE_TYPE, user_setting.MAX_SAVE_CANDLE_COUNT)
    ...
    def requestOpt20005(self, yupjong_code):
        ...
        ret = 0
        if(yupjong_code == '001'):
            ret = self.commRqData(yupjong_code, "opt20005",
                                  0, kw_util.sendReqYupjongKospiScreenNo)
        else:
            ret = self.commRqData(yupjong_code, "opt20005",
                                  0, kw_util.sendReqYupjongKosdaqScreenNo)
    ...
    def requestOpt10001(self, jongmok_code):
        # print(util.cur_time_msec() )
        self.setInputValue("종목코드", jongmok_code)
        ret = self.commRqData(jongmok_code, "opt10001",
                              0, kw_util.sendGibonScreenNo)
        errorString = None
        if(ret != 0):
            errorString = jongmok_code + " commRqData() " + kw_util.parseErrorCode(str(ret))
    ...
    def onTimerSystemTimeout(self):
        ...
        for key, value in user_setting.CONDITION_INFO.items():
            start_time = datetime.time(
                hour=value['start_time']['hour'], minute=value['start_time']['minute'], second=value['start_time']['second'])
            end_time = datetime.time(
                hour=value['end_time']['hour'], minute=value['end_time']['minute'], second=value['end_time']['second'])
        ...
        if(self.getConnectState() != 1):
            ...
            if(datetime.time(*TRADING_INFO_GETTING_TIME) <= self.currentTime.time()):
                ...
                self.make_excel(CHEGYEOL_INFO_EXCEL_FILE_PATH,
                                self.chegyeolInfo)
    ...
    def _OnReceiveMsg(self, scrNo, rQName, trCode, msg):
        ...
        # [100000] 조회가 완료되었습니다
        printData = 'sScrNo: {}, sRQName: {}, sTrCode: {}, sMsg: {}'.format(
    ...
    def _OnReceiveTrData(self, scrNo, rQName, trCode, recordName,
        ...
        if (trCode == 'opw00018'):
            ...
            if(self.makeOpw00018Info(rQName)):
                ...
                if(prevNext == "2"):
                    QTimer.singleShot(20, lambda: self.requestOpw00018(
                        self.account_list[0], prevNext))
                else:
                    QTimer.singleShot(TR_TIME_LIMIT_MS,
                                      self.sigRequestJangoComplete)
    ...
    def _OnReceiveRealData(self, jongmok_code, realType, realData):
        ...
        if(realType == "주식호가잔량"):
            ...
            for col_name in kw_util.dict_jusik['실시간-{}'.format(realType)]:
                result = self.getCommRealData(
                    jongmok_code, kw_util.name_fid[col_name])
            ...
            if('분봉' in upjong):
                ...
                if(current_chegyeol_time_str != '장마감'):
                    current_chegyeol_time = datetime.datetime.strptime(
                        current_chegyeol_time_str, "%H%M%S").time().replace(second=0)
                else:
                    current_chegyeol_time = self.currentTime.time()

                last_chegyeol_time_str = upjong['분봉'][0].split(':')[1]
                last_chegyeol_time = datetime.datetime.strptime(
                    last_chegyeol_time_str, "%Y%m%d%H%M%S")

                time_span = datetime.timedelta(minutes=3)

                if(current_chegyeol_time >= (last_chegyeol_time + time_span).time().replace(second=0)):
                    upjong['분봉'].insert(0, '{}:{}'.format(
                        current_price_str, '19990101{}'.format(current_chegyeol_time_str)))
    ...
    def calculateSuik(self, jongmok_code, current_price, amount):
        ...
        current_jango['수익율'] = round(
            ((maedo_danga - maeip_danga) / maeip_danga) * 100, 2)
        pass
    ...
    def makeRealDataInfo(self, jongmok_code, real_data_type):
        ...
        for col_name in kw_util.dict_jusik[real_data_type]:
            result_list.append(self.getCommRealData(
                jongmok_code, kw_util.name_fid[col_name]).strip())
    ...
    def processStopLoss(self, jongmok_code):
        ...
        # 날짜:가격:수량
        last_bunhal_maesu_time_str = bunhal_maesu_info_list[-1].split(':')[0]
        last_maeip_price = int(
            bunhal_maesu_info_list[-1].split(':')[1])  # 날짜:가격:수량
        ...
        first_bunhal_maesu_date_time = datetime.datetime.strptime(
            first_bunhal_maesu_time_str, '%Y%m%d%H%M%S').date()
        ...
        if(_yesterday_date >= first_bunhal_maesu_date_time):
            ...
            ##########################################################################################################
            # 당일 매수 종목
            last_bunhal_maesu_date_time = datetime.datetime.strptime(
                last_bunhal_maesu_time_str, "%Y%m%d%H%M%S")
            stop_plus = 9999999
    ...
    * 14:03:36.353244 _OnReceiveChejanData gubun: 1, itemCnt: 34, fidList: 9201;9001;917;916;302;10;930;931;932;933;945;946;950;951;27;28;307;8019;957;958;918;990;991;992;993;959;924;10010;25;11;12;306;305;970
    ...
    def _OnReceiveChejanData(self, gubun, itemCnt, fidList):
        ...
        if(gubun == "1"):  # 잔고 정보
            ...
            jumun_ganeung_suryang = int(
                self.getChejanData(kw_util.name_fid['주문가능수량']))
            maeip_danga = int(self.getChejanData(kw_util.name_fid['매입단가']))
            jongmok_name = self.getMasterCodeName(jongmok_code)
            current_price = abs(
                int(self.getChejanData(kw_util.name_fid['현재가'])))
            current_amount = abs(
                int(self.getChejanData(kw_util.name_fid['당일순매수수량'])))
            maesuHoga1 = abs(
                int(self.getChejanData(kw_util.name_fid['(최우선)매수호가'])))
            maedoHoga1 = abs(
                int(self.getChejanData(kw_util.name_fid['(최우선)매도호가'])))
            ...
            # TR 잔고에서 매매가능 수량 이란 이름으로 사용되므로
            current_jango['매매가능수량'] = jumun_ganeung_suryang
            ...
            printData = ''
            if(maemae_type == 1):
                printData = "{}: 매도 주문가능수량:{} / 보유수량:{}".format(
                    jongmok_name, jumun_ganeung_suryang, boyou_suryang)
            else:
                printData = "{}: 매수 주문가능수량:{} / 보유수량:{}".format(
                    jongmok_name, jumun_ganeung_suryang, boyou_suryang)
            ...
            # 매수
            if(maemae_type == 2):
                chegyeol_info = util.cur_date_time(
                    '%Y%m%d%H%M%S') + ":" + str(maeip_danga) + ":" + str(current_amount)
                if(jongmok_code not in self.jangoInfo):
                    # 첫매수
                    ...
                    if(last_price != current_price):
                        chegyeol_info_list = self.jangoInfo[jongmok_code].get(
                            '분할매수이력', [])
            ...
            # 매도
                ...
                if(boyou_suryang == 0):
                    ...
                    chegyeol_info = util.cur_date_time(
                        '%Y%m%d%H%M%S') + ":" + str(current_price) + ":" + str(current_amount)
            ...
            jongmok_code = self.getChejanData(kw_util.name_fid['종목코드'])[1:]
            jongmok_name = self.getMasterCodeName(jongmok_code)
            michegyeol_suryang = int(
                self.getChejanData(kw_util.name_fid['미체결수량']))
            ...
            printData = ''
            if(maemae_type == 1):
                printData = "{}: 매도 {} 미체결수량 {}".format(
                    jongmok_name, jumun_sangtae, michegyeol_suryang)
            else:
                printData = "{}: 매수 {} 미체결수량 {}".format(
                    jongmok_name, jumun_sangtae, michegyeol_suryang)
            ...
            if(jumun_sangtae == "체결"):
                ...
                # 매도 접수인 경우
                if(jongmok_code in self.jangoInfo):
                    print("sell: {} ordernumber: {} 접수 ".format(
                        jongmok_name, jumun_number))
    ...
    def makeChegyeolInfoFile(self):
        ...
        with open(CHEGYEOL_INFO_FILE_PATH, 'w', encoding='utf8') as f:
            f.write(json.dumps(self.chegyeolInfo,
                    ensure_ascii=False, indent=2, sort_keys=True))
        pass
    ...
    def makeEtcJangoInfo(self, jongmok_code):
        ...
        # 기본 손절가 측정
        gibon_stoploss = round(
            maeip_price * (1 + (stop_loss_percent / 100)), 2)

        print("종목이름:{}, 기본손절:{}".format(
            self.getMasterCodeName(jongmok_code), gibon_stoploss))

        ###############################################################################################
        current_jango['손절가'] = gibon_stoploss
        current_jango['이익실현가'] = round(
            maeip_price * (1 + (stop_plus_percent/100)), 2)
    ...
    def makeChegyeolInfo(self, jongmok_code, fidList):
        ...
        if(maedo_maesu_gubun == '매도'):
            # 체결가를 통해 수익율 필드 업데이트
            current_price = int(self.getChejanData(
                kw_util.name_fid['체결가']).strip())
            current_maedo_amount = int(self.getChejanData(
                kw_util.name_fid['주문수량']).strip())

            self.calculateSuik(jongmok_code, current_price,
                               current_maedo_amount)
    ...
    def _OnReceiveTrCondition(self, scrNo, codeList, conditionName, index, next):
        ...
        if('이탈' not in conditionName):
            for code in codes:
                print('condition list add: {} '.format(
                    code) + self.getMasterCodeName(code))
    ...
    def _OnReceiveRealCondition(self, code, type, conditionName, conditionIndex):
        ...
        if ('이탈' in conditionName):
            ...
            if (type == 'I'):
                ...
                QTimer.singleShot(
                    10, lambda: self.removeConditionOccurList(code))
                pass
    ...
    def refreshRealRequest(self):
        ...
        if(len(codeList)):
            ...
            tmp = self.setRealReg(kw_util.sendRealRegHogaScrNo, ';'.join(
                codeList), kw_util.type_fidset['주식호가잔량'], "0")
            if(tmp < 0):
                print("주식호가잔량: " + kw_util.parseErrorCode(tmp))

            tmp = self.setRealReg(kw_util.sendRealRegChegyeolScrNo, ';'.join(
                codeList), kw_util.type_fidset['주식체결'], "0")
            if(tmp < 0):
                print("주식체결: " + kw_util.parseErrorCode(tmp))

            # 주식종목정보 신청시 주식 당일 거래원 정보 올라옴
            # 주식종목정보 실시간의 경우 올라오는 시간이  비주기적임
            tmp = self.setRealReg(kw_util.sendRealRegSiseSrcNo, ';'.join(
                codeList), kw_util.type_fidset['주식종목정보'], "0")
            if(tmp < 0):
            ...
            tmp = self.setRealReg(
                kw_util.sendRealRegUpjongScrNo, '001;101', kw_util.type_fidset['업종지수'], "0")
            if(tmp < 0):
        ...
        for date_str, value in sorted(data_dict.items()):
            ...
            if(sheet_name not in sheet_names):
                new_sheet = wb.add_worksheet(
                    title=sheet_name, rows=100, cols=20, index=0)
            else:
                continue
            ...
            for row_index, line in enumerate(value):
                ...
                range_str = "A{}:{}{}".format(
                    row_index + 1, chr(ord('A') + len(items)),  row_index + 1)
            ...
            for plus_range in stop_plus_range_list:
                new_sheet.format(plus_range, {"backgroundColor": {
    ...
    def getMasterStockInfo(self, strCode):
        stock_info = self.ocx.dynamicCall(
            "KOA_Functions(QString, QString)", "GetMasterStockInfo", strCode)
...
if __name__ == "__main__":
    def test_opw00004():
        QTimer.singleShot(10, lambda: objKiwoom.requestOpw00004(
            objKiwoom.account_list[0]))

    def test_opw00005():
        QTimer.singleShot(10, lambda: objKiwoom.requestOpw00005(
            objKiwoom.account_list[0]))
    ...
    logging.basicConfig(filename='system_err.log', filemode='a',
